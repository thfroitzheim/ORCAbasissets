...
def orcaecpcode(ecp):
    ...
    for i in range(0,len(ecp["symb"])):
        l = 0
        ofile.write("  // --------------------------------------------------------\n")
        ofile.write("  // element no "+str(ecp["numb"][i])+" ("+ecp["symb"][i]+")\n")
        ofile.write("  // --------------------------------------------------------\n")
        ofile.write("  E->U[ {0:3d} ] = new ecpPotential;\n".format(int(ecp["numb"][i])))
        ofile.write("  E->U[ {0:3d} ]->initialize({1:4d},{2:4d},{3:4d} );\n".format(ecp["numb"][i],int(angmomdict[ecp["lmax"][i].upper()]),ecp["ncore"][i],ecp["nbf"][i]))
        ofile.write("  E->U[ {0:3d} ]->element    ={0:4d};\n".format(ecp["numb"][i]))
        ofile.write("  CopyECPCommentary( E->U[ {0:3d} ], all_ecpname, all_comment, all_citation, all_source );\n".format(ecp["numb"][i]))
        ...
        for j in range(0,ecp["nbf"][i]):
            ...
            for k in range(0,ecp["lnpr"][i][j]):
                ofile.write("  EG[{0:4d} ].p[{1:4d} ].a ={2:23.12f} ;".format(j,k,ecp["exponents"][i][l]))
                ofile.write("  EG[{0:4d} ].p[{1:4d}  ].d ={2:23.12f} ;".format(j,k,ecp["coefficients"][i][l]))
                ofile.write("  EG[{0:4d} ].p[{1:4d}  ].n ={2:4.1f} ;\n".format(j,k,float(ecp["ecpnfactor"][i][l])))
                l+=1
        ofile.write("\n\n")